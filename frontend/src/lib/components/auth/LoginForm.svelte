<script lang="ts">
	...
	async function handleMFASubmit(e: Event) {
		...
		try {
			const response = await authApi.verifyMFA({ code: mfaCode, session_id: sessionId });
			authStore.setUser(response.user);
			goto('/dashboard');
		} catch (err: unknown) {
		...
		{:else}
			<form onsubmit={handleMFASubmit} class="space-y-4">
				...
				<div class="flex gap-3">
					<Button type="button" variant="outline" fullWidth onclick={() => (showMFA = false)}>
						Back
					</Button>
					<Button type="submit" {loading} fullWidth>Verify</Button>
				</div>
			</form>
		{/if}
	</div>
